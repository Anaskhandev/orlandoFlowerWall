...
class Light_up_card extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $content['records'] = $this->db->select('*')->from('light_up_card')
    ...
    public function form($form_type = "", $id = "")
    {
        ...
        if ($_POST) {
            ...
            if (!$this->form_validation->run() == TRUE) {
                ...
                $content['record']  = $this->db->select('*')->from('light_up_card')
                    ->where(array('light_up_card_id' => $id))->get()->result();
                echo print_r($content);
                exit;
                $content['list']  = $this->db->select('*')->from('light_up_card')
                    ->where(array('light_up_card_status' => 'enable'))->get()->result();
    ...
    public function form2($form_type = "", $id = "")
    {
        if (!$this->form_validation->run() == TRUE) {
            ...
            $data['table'] = 'light_up_card';
            if ($form_type == 'edit') {
                $data['where'] = array('light_up_card_id' => $id);
                $this->general->update($data, $content);
                $this->session->set_flashdata('success', 'Updated Successfully.');
                redirect('admin/' . $this->table_name);
            } else if ($form_type == 'add' || $form_type == 'duplicate') {
                $this->db->insert('light_up_card', $content);
                // $this->general->insert($data, $content);
                $this->session->set_flashdata('success', 'Added Successfully.');
                redirect('admin/' . $this->table_name);
            }
