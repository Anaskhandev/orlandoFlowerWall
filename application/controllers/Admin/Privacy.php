...
class privacy extends Admin_Controller
{
    ...
    public function form($form_type = "", $id = "")
    {
        ...
        if ($_POST) {
            ...
            if (!$this->form_validation->run() == TRUE) {
                ...
                $content['record']  = $this->db->select('*')->from('privacy')
                    ->where(array('privacy_id' => $id))->get()->result();
                echo print_r($content);
                exit;
                $content['list']  = $this->db->select('*')->from('privacy')
                    ->where(array('privacy_status' => 'enable'))->get()->result();
    ...
    public function form2($form_type = "", $id = "")
    {
        if (!$this->form_validation->run() == TRUE) {
            ...
            $data['table'] = 'privacy';
            if ($form_type == 'edit') {
                $data['where'] = array('privacy_id' => $id);
                $this->general->update($data, $content);
                $this->session->set_flashdata('success', 'Updated Successfully.');
                redirect('admin/' . $this->table_name);
            } else if ($form_type == 'add' || $form_type == 'duplicate') {
                $this->db->insert('privacy', $content);
                // $this->general->insert($data, $content);
                $this->session->set_flashdata('success', 'Added Successfully.');
                redirect('admin/' . $this->table_name);
            }
