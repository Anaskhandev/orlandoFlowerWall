...
class category_list extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $content['records'] = $this->db->select('*')->from('category_list')
    ...
    public function form($form_type = "", $id = "")
    {
        ...
        if ($_POST) {
            ...
            if (!$this->form_validation->run() == TRUE) {
                ...
                $content['record']  = $this->db->select('*')->from('category_list')
                    ->where(array('category_list_id' => $id))->get()->result();
    ...
    public function form2($form_type = "", $id = "")
    {
        if (!$this->form_validation->run() == TRUE) {
            ...
            $data['table'] = 'category_list';
            if ($form_type == 'edit') {
                $data['where'] = array('category_list_id' => $id);
                $this->general->update($data, $content);
                $this->session->set_flashdata('success', 'Updated Successfully.');
                redirect('admin/' . $this->table_name);
            } else if ($form_type == 'add' || $form_type == 'duplicate') {
                $this->db->insert('category_list', $content);
                // $this->general->insert($data, $content);
                $this->session->set_flashdata('success', 'Added Successfully.');
                redirect('admin/' . $this->table_name);
            }
