...
$.widget('wx.timepicker', {
  ...
  _refreshClock: function () {
    ...
    this.timepicker
      .find('.hand.hour')
      .css('transform', 'rotate(' + (this.hour / 12) * 360 + 'deg)');
    this.timepicker
      .find('.hand.minute')
      .css('transform', 'rotate(' + (this.minute / 60) * 360 + 'deg)');
  },
  ...
  _parseInput: function () {
    ...
    if (
      time.length > 0 &&
      (match = /^(\d{1,2})(?::?(\d{2}))?(?: ?([ap])\.?(?:m\.?)?)?$/i.exec(time))
    ) {
      valid = true;
  ...
  _buildMinutes: function () {
    ...
    this.timepicker
      .find('.minute.hand')
      .addClass('selected')
      .appendTo(this.timepicker.find('.face'));
