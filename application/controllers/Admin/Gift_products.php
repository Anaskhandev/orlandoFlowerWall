...
class Gift_products extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $content['records'] = $this->db->select('*')->from('gift_products')
            ->where(array('gift_products_status' => 'enable'))->get()->result();
    ...
    public function form($form_type = "", $id = "")
    {
        ...
        if ($_POST) {
            ...
            if (!$this->form_validation->run() == TRUE) {
                ...
                $content['record']  = $this->db->select('*')->from('gift_products')
                    ->where(array('gift_products_id' => $id))->get()->result();
    ...
    public function form2($form_type = "", $id = "")
    {
        if (!$this->form_validation->run() == TRUE) {
            ...
            $data['table'] = 'gift_products';
            if ($form_type == 'edit') {
                $data['where'] = array('gift_products_id' => $id);
                $this->general->update($data, $content);
                $this->session->set_flashdata('success', 'Updated Successfully.');
                redirect('admin/' . $this->table_name);
            } else if ($form_type == 'add' || $form_type == 'duplicate') {
                $this->db->insert('gift_products', $content);
                // $this->general->insert($data, $content);
                $this->session->set_flashdata('success', 'Added Successfully.');
                redirect('admin/' . $this->table_name);
            }
