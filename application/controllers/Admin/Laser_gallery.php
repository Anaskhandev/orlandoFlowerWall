...
class Laser_gallery extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $content['records'] = $this->db->select('*')->from('laser_gallery')
            ->where(array('laser_gallery_status' => 'enable'))->get()->result();
    ...
    public function form($form_type = "", $id = "")
    {
        ...
        if ($_POST) {
            ...
            if (!$this->form_validation->run() == TRUE) {
                ...
                $content['record']  = $this->db->select('*')->from('laser_gallery')
                    ->where(array('laser_gallery_id' => $id))->get()->result();
    ...
    public function form2($form_type = "", $id = "")
    {
        if (!$this->form_validation->run() == TRUE) {
            ...
            $data['table'] = 'laser_gallery';
            if ($form_type == 'edit') {
                $data['where'] = array('laser_gallery_id' => $id);
                $this->general->update($data, $content);
                $this->session->set_flashdata('success', 'Updated Successfully.');
                redirect('admin/' . $this->table_name);
            } else if ($form_type == 'add' || $form_type == 'duplicate') {
                $this->db->insert('laser_gallery', $content);
                // $this->general->insert($data, $content);
                $this->session->set_flashdata('success', 'Added Successfully.');
                redirect('admin/' . $this->table_name);
            }
